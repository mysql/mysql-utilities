...
"""
This module contains an abstraction of a topolgy object used to discover
slaves and down-stream replicants.
"""
...
class Topology(object):
    """The Topology class can be used to connect to a running MySQL server
    ...
    To generate a topology, the caller must call the generate_topology() method
    to build the topology. This is left as a separate state because it can be
    a lengthy process thereby too long for a constructor method.
    ...
    def _get_slaves(self, master):
        """Return the topology for this master.
        
        This method returns a list of slaves (host, port) if this server is
        a master in a replication topology.

        master[in]           The server (master) to check for slaves

        Returns list - (master:port, [master:port, [...]])
        """
        ...
        res = master.exec_query("SHOW SLAVE HOSTS")
        if not res == []:
    ...
    def _get_topology(self, max_depth, seed_conn=None, masters_found=[]):
        """Find the attached slaves for a list of server connections.
        ...
        slaves = self._get_slaves(master)
        slave_list = []
        depth = 0
        if len(slaves) > 0:
            for slave in slaves:
                ...
                if self.recurse and not slave in masters_found and \
                   ((max_depth is None) or (depth < max_depth)):
                    new_list = self._get_topology(max_depth, slave_conn,
                                                  masters_found)
                    if new_list == []:
    ...
    def generate_topology(self, max_depth):
        """Find the attached slaves for a list of server connections.
        ...
        max_depth[in]       Maximum depth of recursive search
        """
        self.topology = self._get_topology(max_depth)
    ...
    def print_graph(self, topology_list=[], masters_found=[],
                    level=0, preamble=""):
        """Prints a graph of the topology to standard output.
    ...
    def _get_row(self, topology_list):
        """Get a row (master, slave) for the topology.
    ...
    def get_topology_list(self):
        """Get a list of the topology suitable for export
