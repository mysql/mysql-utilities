...
::
  ...
  mysqldiff --server1=<user>[<passwd>]@<host>:[<port>][:<socket>]
            [ --server2=<user>[<passwd>]@<host>:[<port>][:<socket>] |
              ...
              --quiet | --format=[unified|context|differ]
              [<db1:db2> [<db1:db2>*] |
...
--server1 option. The user can also connect to another server by specifying
the --server2 option. In this case, the database or database object pair
align such that database1 (or database1.object1) are taken from server1 and
database2 (or database2.object2) are taken from server2.
...
--force option will will attempt to compare all objects listed as arguments.
...
.. option:: -f --format <diff-format>

   Display the differences in either **unified** (default), **context** or
   **differ** format
...
use this command::
    ...
    $python ./scripts/mysqldiff.py --server1=root@localhost employees:emp1 
    # server1 on localhost: ... connected.
    WARNING: Objects in server1:employees but not in server2:emp1:
      EVENT: e1
    WARNING: Objects in server2:emp1 but not in server1:employees:
    Diff failed. One or more differences found.
    
    $ python ./scripts/mysqldiff.py --server1=root@localhost \
    ...
    $ python ./scripts/mysqldiff.py --server1=root@localhost \
             employees.salaries:emp1.salaries --format=differ
    # server1 on localhost: ... connected.
