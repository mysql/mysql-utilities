...
SLOW_LOG_ENTRIES = [
    ...
    ('# User@Host: a[b] @ c.d [127.0.0.1]',
     ...
     'SELECT 1;'),
    ('# User@Host: a[b] @ c.d [127.0.0.1]  Id:     1',
     '# Query_time: 0.000059  Lock_time: 0.000019'
        'Rows_sent: 3  Rows_examined: 3',
     'SET timestamp=1338440410;',
     'SELECT 1;')
...
class TestSlowQueryLog(BaseParserTestCase):
    ...
    def test_parse_connection_info(self):
        ...
        tests = [
            ('# User@Host: root[root] @ localhost []',
             dict(user='root', host='localhost', session_id=None)),
            ('# User@Host: root[root] @ localhost [127.0.0.1]',
             dict(user='root', host='localhost', session_id=None)),
            ('# User@Host: root[root] @ [127.0.0.1]',
             dict(user='root', host='127.0.0.1', session_id=None)),
            ('# User@Host: [ham] @ localhost []',
             dict(user='ham', host='localhost', session_id=None)),
            ('# User@Host: [ham] @ [127.0.0.1]',
             dict(user='ham', host='127.0.0.1', session_id=None)),
        ]
