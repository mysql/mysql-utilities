...
class test(mutlib.System_test):
    ...
    def _test_server_clone(self, cmd_str, comment, kill=True,
        ...
        if kill:
            ...
            self.servers.stop_server(self.new_server, drop=drop)
            # if restart, try to use the generated script to restart the server.
            if restart:
                if os.name == 'posix':
                ...
                with open("restart.txt", 'a+') as f_out:
                    if self.debug:
                        ...
                        proc = subprocess.Popen(run_script, stdout=f_out,
                                                stderr=f_out)
