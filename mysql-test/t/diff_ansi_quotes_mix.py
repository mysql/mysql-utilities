...
"""
diff test with sql_mode=ANSI_QUOTES and different sql_mode.
"""
...
class test(diff.test):
    ...
    One server is set with sql_mode=ANSI_QUOTES.
    """
    ...
    def setup(self):
        mysqld = '"--report-port={0}"'.format(self.servers.view_next_port())
        self.server1 = self.servers.spawn_server("diff_srv1_ansi_quotes",
                                                 mysqld, True)
        mysqld = _DEFAULT_MYSQL_OPTS.format(self.servers.view_next_port())
        self.server2 = self.servers.spawn_server("diff_srv2_ansi_quotes",
                                                 mysqld, True)

        self.drop_all()
        data_file = os.path.normpath("./std_data/basic_data_ansi_quotes.sql")
        try:
        ...
        data_file_backticks = os.path.normpath(
            "./std_data/db_compare_backtick_ansi_quotes.sql")
        try:
    ...
    def run(self):
        ...
        test_num += 1
        comment = ("Test case {0} - diff multiple objects + database - same "
                   "name".format(test_num))
        ...
        if os.name == 'posix':
            ...
            cmd_arg = '"`db.``:db`:\\"db.`:db\\""'
            # Execute test (differences expected on name only due sql_mode)
        test_num += 1
        comment = ("Test case {0} - diff a database with objects of "
                   "different types with differences on name only due sql_mode"
                   "".format(test_num))
        cmd = "mysqldiff.py {0} {1} {2} {3}".format(s1_conn, s2_conn, cmd_arg,
                                                    "--force")
        res = self.run_test_case(1, cmd, comment)
        if not res:
            raise MUTLibError("{0}: failed".format(comment))
